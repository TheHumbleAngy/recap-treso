                ...
                if ($resultat = mysqli_query($connection, $sql_grp_banq)) {
                    if ($resultat->num_rows > 0) {
                        ...
                        foreach ($lignes as $ligne) { // banque-pays
                            ...
                            $libelle_pays[$n][0] = stripcslashes($ligne['libelle_pays']);

                            $libelle_b = addslashes($libelle_banque[$n]);
                            $libelle_p = addslashes($libelle_pays[$n][0]);
                    ...
                    FROM banques b
                      ...
                      INNER JOIN monnaies m on bpm.id_monnaie = m.id_monnaie
                      INNER JOIN pays p ON bpm.id_pays = p.id_pays
                      WHERE libelle_banque = '$libelle_b' AND libelle_pays = '$libelle_p'
...
<table class="table table-sm table-hover my-4 ncare font-weight-light" id="etat" style="font-size: small">
    ...
    <tr class="">
        <th class="col-3" rowspan="2">' . $libelle_banque[$n] . ' ' . $libelle_pays[$n][0] . '</th>
                            ...
                            if ($resultat = mysqli_query($connection, $sql_grp_curr)) {
                                if ($resultat->num_rows > 0) {
                                    ...
                                    foreach ($rows as $row) { // banque-pays-monnaie
                                        $m++;
                                        $libelle_pays[$n][$m] = stripcslashes($row['sigle_monnaie']);

                                        $banque = $libelle_banque[$n];
                                        $pays = $libelle_pays[$n][0];
                                        $monnaie = $libelle_pays[$n][$m];
                                ...
                                FROM operations o 
                                  ...
                                  INNER JOIN pays p ON bpm.id_pays = p.id_pays
                                  INNER JOIN monnaies m on bpm.id_monnaie = m.id_monnaie
                                 WHERE o.id_type_operation = 1 AND libelle_banque = '$banque' AND libelle_pays = '$pays' 
                                  AND sigle_monnaie = '$monnaie' AND date_saisie_operation < '$debut'";
                                ...
                                FROM operations o 
                                  ...
                                  INNER JOIN pays p ON bpm.id_pays = p.id_pays
                                  INNER JOIN monnaies m on bpm.id_monnaie = m.id_monnaie
                                 WHERE o.id_type_operation = 0 AND libelle_banque = '$banque' AND libelle_pays = '$pays' 
                                  AND sigle_monnaie = '$monnaie' AND date_saisie_operation < '$debut'";
                                ...
                                FROM operations o 
                                  ...
                                  INNER JOIN monnaies m on bpm.id_monnaie = m.id_monnaie
                                 WHERE o.id_type_operation = 1 AND libelle_banque = '$banque' AND libelle_pays = '$pays' 
                                  AND sigle_monnaie = '$monnaie' AND date_saisie_operation >= '$debut' AND date_saisie_operation <= '$fin'";
                                ...
                                //echo "Solde prec: " . $solde_prec . " Entrées: " . $rec_apr . " Sortie: " . $dep_apr . " Solde final: " . $solde_final . "<br><br>";
                                        ...
                                        echo '
<tr>
    <td class="montant pl-4">' . $monnaie . '</td>
...
<tr class="table-success" style="font-weight: bold;">
    <td class="montant">Total ' . $libelle_banque[$n] . ' ' . $libelle_pays[$n][0] . '</td>
    <td class="montant text-right"></td>
    <td class="montant text-right"></td>
        ...
        <th></th>
            ...
            elseif (isset($_POST['rdo_nature']) && $_POST['rdo_nature'] == "selection" && isset($_POST['liste'])) {
                ...
                foreach ($arr as $item) {
                    ...
                    $libelle_pays[$n][0] = $tab[1];

                    $banque = addslashes($tab[0]);
                    $pays = addslashes($tab[1]);
...
FROM banques b
  ...
  INNER JOIN monnaies m on bpm.id_monnaie = m.id_monnaie
  INNER JOIN pays p ON bpm.id_pays = p.id_pays
WHERE libelle_banque = '$libelle_banque[$n]' AND libelle_pays = '$pays'
ORDER BY sigle_monnaie";
                    ...
                    if ($resultat = mysqli_query($connection, $sql_grp_curr)) {
                        if ($resultat->num_rows > 0) {
                            ...
                            foreach ($rows as $row) { // banque-pays-monnaie
                                ...
                                $banque = addslashes($libelle_banque[$n]);
                                $pays = addslashes($libelle_pays[$n][0]);
                                $monnaie = $libelle_pays[$n][$m];
...
FROM operations o
  ...
  INNER JOIN monnaies m on bpm.id_monnaie = m.id_monnaie
WHERE o.id_type_operation = 1 AND libelle_banque = '$banque' AND libelle_pays = '$pays'
      AND sigle_monnaie = '$monnaie' AND date_saisie_operation >= '$debut' AND
