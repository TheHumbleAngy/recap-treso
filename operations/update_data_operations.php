<?php
     ...
     * Time: 5:55 PM
     */
    if (isset($_GET['action']) && $_GET['action'] == "ajout_operation" && isset($_POST['monnaie']) && isset($_POST['pays'])) {
        include 'class_operations.php';

        $operation = new operations();

        $id_banque = htmlspecialchars($_POST['id_banque'], ENT_QUOTES);
        $id_pays = htmlspecialchars($_POST['pays'], ENT_QUOTES);
        $monnaie = htmlspecialchars($_POST['monnaie'], ENT_QUOTES);
        ...
        $sql = "SELECT id_monnaie FROM monnaies WHERE sigle_monnaie = '$monnaie'";
        $resultat = $connection->query($sql);
        $id_monnaie = "";
        if ($resultat->num_rows > 0) {
            $lignes = $resultat->fetch_all(MYSQLI_ASSOC);
            foreach ($lignes as $ligne) {
                $id_monnaie = stripslashes($ligne['id_monnaie']);
            }
        }
        ...
        for ($i = 0; $i < $nbr; $i++) {
            ...
            if ($operation->setData($id_type_operation, $id_banque, $id_pays, $id_monnaie, $compte_operation[$i], $tag_operation[$i], $date_saisie_operation, $date_operation[$i], $designation_operation[$i], $cours_operation[$i], $montant_operation[$i], $montant_xof_operation[$i], $statut_operation[$i], $observation_operation[$i])) {
        ...
        if ($operation->updateStatut($id, $statut)) {
